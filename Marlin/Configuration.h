...
#define STRING_CONFIG_H_AUTHOR "(Miroslav Jasso, SKR Mini E3 v3)" // Original author or contributor.
 ...
 */
#define DEFAULT_AXIS_STEPS_PER_UNIT   { 80.01, 80.15, 401.2, 398.47 }
 ...
 *                                      X, Y, Z [, I [, J [, K...]]], E0 [, E1[, E2...]]
 */
#define DEFAULT_MAX_ACCELERATION      { 2000, 2000, 100, 600 }
 ...
 *     O-- FRONT --+
 */
#define NOZZLE_TO_PROBE_OFFSET { -31.4, -40.1, -1.67 } // (mm) X, Y, Z distance from Nozzle tip to Probe trigger-point
...
#define PROBE_OFFSET_XMIN -100   // (mm)
...
//#define DEBUG_LEVELING_FEATURE
  ...
  #define AVOID_OBSTACLES
    ...
    #define OBSTACLE1 { (X_BED_SIZE) / 2     - (CLIP_W) / 2,                       0, (X_BED_SIZE) / 2     + (CLIP_W) / 2, CLIP_H }
    //#define OBSTACLE2 { (X_BED_SIZE) * 3 / 4 - (CLIP_W) / 2,                       0, (X_BED_SIZE) * 3 / 4 + (CLIP_W) / 2, CLIP_H }
    #define OBSTACLE3 { (X_BED_SIZE) / 2     - (CLIP_W) / 2, (Y_BED_SIZE) - (CLIP_H), (X_BED_SIZE) / 2     + (CLIP_W) / 2, Y_BED_SIZE }
